...
app.get('/api/run-scraper', async (req, res) => {
  ...
  const repo = 'FAMBotPage';
  const workflowId = 'sync-fam.yml';
  ...
  try {
    ...
    if (conclusion === 'success') {
      sendSSE('status', { message: 'Cloud scraper completed successfully! Pulling latest FAM rates to local machine...' });
